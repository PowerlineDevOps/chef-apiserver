...
default[:web][:user] = 'civix'
default[:web][:group] = 'civix'
default[:aws_access_key] = 'xxx'
default[:aws_secret_key] = 'yyy'
default[:s3_deployment_bucket] = 'deployment-data'
default[:deploy][:deploy_to] = '/srv/civix/web'

# ================ deploy =============
default[:deploy][:repo] = 'https://github.com/PowerlineApp/powerline-server.git'
default[:deploy][:shell] = '/bin/bash'
default[:deploy][:deploy_keep_releases] = 5
default[:deploy][:deploy_to] = '/srv/civix/web'
default[:deploy][:branch] = 'develop'
default[:deploy][:staging_dir] = "/srv/staging"
